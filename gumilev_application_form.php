...
<body>
    ...
    <div class="container">
        <h1 class="text-center my-5">L.N.Gumilyov Eurasian National University Application Form</h1>
        ...
        if (mysqli_num_rows($result_check) > 0) {
            ...
            if (strpos($sub_universities, 'gumi') !== false) {
                echo "<p class='alert alert-danger'>You have already submitted an application for Gumilev University.</p>";
            } 
                ...
                try {
                    ...
                    $sql_update = "UPDATE users SET submissions = submissions + 1, sub_universities = CONCAT(sub_universities, 'gumi,') WHERE email = '$email'";
                    ...
                    if (move_uploaded_file($hs_diploma_tmp, $hs_diploma_path) && move_uploaded_file($eng_cert_tmp, $eng_cert_path)) {
                        $sql_insert = "INSERT INTO gumilev_applications (user_id, full_name, dob, email, phone, id_code, edu_language, hs_diploma, eng_cert) VALUES ('$user_id', '$full_name', '$dob', '$email', '$phone', '$id_code', '$edu_language', '$hs_diploma_path', '$eng_cert_path')";
                        mysqli_query($mysqli, $sql_insert);
                        $sql_insert_2 = "UPDATE universities SET user_id = CONCAT(user_id, '$user_id,') WHERE id = '2'";
                        mysqli_query($mysqli, $sql_insert_2);
                        $sql_insert_status = "INSERT INTO applications_status (user_id, user_status, university_name) VALUES ('$user_id', 'PROCESSING', 'gumi')";
